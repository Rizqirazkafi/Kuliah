...
void searchit(tree **root, string cari) {
  if ((*root) == NULL)
    ...
    return (searchit(&(*root)->left, cari));
  else if (cari > (*root)->nim)
    return (searchit(&(*root)->right, cari));
  else if (cari == (*root)->nim) {
    cout << "Data ditemukan\n";
...
int countit(tree *root) {
  if (root == NULL)
    ...
    return countit(root->left) + countit(root->right) + 1;
}
...
int main() {
  ...
  while ((ulang == 'y') || (ulang == 'Y')) {
    // clear terminal screen for windows powershell and cmd prompt
    system("cls");
    // clear terminal screen for UNIX based terminal (Linux and MacOS)
    system("clear");
